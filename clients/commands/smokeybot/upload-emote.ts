...
async function uploadEmote(
	...
	if (emote.animated){
		...
		await create_emoji('https:' + emote.host.url + '/2x.gif', interaction, emote.name, 'https:' + emote.host.url + '/1x.gif')

	}else{

		await create_emoji('https:' + emote.host.url + '/2x.png', interaction, emote.name)
...
async function create_emoji(
	...
	try {
	  if (
		await interaction.guild.emojis
		  ...
		  .then(async (emoji) => {
			...
			interaction.editReply(`Successfully uploaded emote \`${emoji.name}\`.`)
	  ...
	  if (err.message.match("Failed to resize asset")) {
		...
		await interaction.editReply(`'${name}' is too big, SmokeyBot will attempt to upload the 1x (smallest) version but if that is also too big it won't resize and will fail to upload.  Note this will also make it lower quality.`);
		...
		try {
			...
			await interaction.editReply(`Ok we can't upload \`${name}\`, likely because it's filesize is still too big.`)
		}

		return false;
	  } else if (err.message.match("Maximum number")) {
		logger.debug(`Maximum number of emotes reached in ${interaction.guild.name}.`);
		await interaction.editReply(`You've reached the maximum amount of emotes for the server.  Make sure you have enough animated AND standard emote slots.  Either one being full will prevent the bot from uploading emotes.`)

		return false;
	  } else if (err.message.match("Missing Permissions")) {
		logger.debug(`Missing Permissions in ${interaction.guild.name}.`);
		await interaction.editReply(`SmokeyBot doesn't have the proper permissions. Make sure SmokeyBot can Manage Emoji in the roles section.`)
