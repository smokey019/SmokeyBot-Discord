...
export async function checkMonstersNew(
  ...
  const sort = interaction.options.get("options").value?.toString();
...
export async function checkMonsters(
  ...
  if (pokemon.length > 0) {
    ...
    const embed = new EmbedBuilder()
      .setAuthor({
        name: `${interaction.user.username}'s Pokémon\nShowing: ${
          format_number(message_contents.length) +
          "/" +
          format_number(pokemon.length)
        }`,
...
export async function checkPokedex(
  ...
  const missing = interaction.options.get("missing").value?.valueOf();
  ...
  const embed = new EmbedBuilder()
    .setAuthor({
      name: `Pokédex - Total Pokémon: ${pokemon_count}`,
      iconURL: interaction.user.avatarURL()?.toString(),
      url: `https://bot.smokey.gg/user/${interaction.user.id}/pokemon`,
    })
    .setDescription(new_msg);
