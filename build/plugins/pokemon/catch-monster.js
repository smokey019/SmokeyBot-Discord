...
function monsterMatchesPrevious(messageContent, { name }) {
  const split = (0, utils_1.explode)(
    messageContent.replace(/ {2,}/gm, ' '),
    ' ',
    2,
  );
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (
      ...
      logger.trace(
        ...
        } - ${message.author.username} | Starting catch~`,
      );
      let level = 0;
      ...
      try {
        ...
        const insertMonster = yield (0, database_1.databaseClient)(
        ...
        if (!updateUser) {
          logger.debug(
            `${
              (_b = message.guild) === null || _b === void 0 ? void 0 : _b.name
            } - ${
              message.author.username
            } | Couldn't update user, insert to user DB~`,
          );
          yield (0, database_1.databaseClient)(
            MonsterUser_1.MonsterUserTable,
          ).insert({
        ...
        if (insertMonster) {
          ...
          if (shiny == 1 && !dex.includes(currentSpawn.id)) {
            ...
            logger.error(
              `'${
                (_c = message.guild) === null || _c === void 0
                  ? void 0
                  : _c.name
              }' - Caught A SHINY POKéMON~ -> '${message.author.username}'`,
            );
            ...
            response = `**${
              random_grats[
                (0, utils_1.getRndInteger)(0, random_grats.length - 1)
              ]
            ...
            logger.info(
              `'${
                (_d = message.guild) === null || _d === void 0
                  ? void 0
                  : _d.name
              }' - Caught POKéMON~ -> '${message.author.username}'`,
            );
            yield (0, database_1.databaseClient)(MonsterUser_1.MonsterUserTable)
              .where({ uid: message.author.id })
