    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (splitMsg.length > 1) {
            if (splitMsg[1] == 'enable') {
                ...
                if (!monsterChannel) {
                    yield message.reply(`You cannot enable smokeMon unless you have a channel called \`pokémon-spawns\` (with the special é). Make sure SmokeyBot has access to read/write in this channel as well.`);
                    return;
                }
                ...
                if (updateGuild) {
                    logger.info(`SmokeMon enabled in ${message.guild.name} | ${message.guild.id}.`);
                    message.reply('smokeMon enabled! This plugin is for fun and SmokeyBot does not own the rights to any images/data and images/data are copyrighted by the Pokémon Company and its affiliates.');
                    ...
                    logger.error(`Couldn't update settings for guild ${message.guild.name} - ${message.guild.id}.`);
                    return false;
                }
            }
            if (splitMsg[1] == 'disable') {
                ...
                if (updateGuild) {
                    logger.info(`smokeMon disabled in ${message.guild.name} | ${message.guild.id}.`);
            ...
            logger.debug(`Not enough parameters for smokemon toggle in ${message.guild.name} | ${message.guild.id}.`);
            return false;
        }
    });
}
